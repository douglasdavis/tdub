...
def setup_logging():
    ...
    # fmt: off
    logging.basicConfig(
        level=logging.INFO, format="%(name)-15s %(funcName)-22s %(levelname)-8s    %(message)s"
    )
    logging.addLevelName(logging.WARNING, "\033[1;31m{}\033[1;0m".format(logging.getLevelName(logging.WARNING)))
    logging.addLevelName(logging.ERROR, "\033[1;35m{}\033[1;0m".format(logging.getLevelName(logging.ERROR)))
